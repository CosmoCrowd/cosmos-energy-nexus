  ...
  return (
    <div className="min-h-screen p-4 pb-24">
      ...
      <div className="cosmic-card rounded-xl p-4 mb-6">
        <h3 className="text-white font-semibold mb-4">Мои достижения</h3>
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center">
            <div className="text-2xl mb-1">🏆</div>
            <div className="text-lg font-bold text-neon-green">#{myRank}</div>
            <div className="text-xs text-gray-400">Место в рейтинге</div>
          </div>
          <div className="text-center">
            <div className="text-2xl mb-1">💎</div>
            <div className="text-lg font-bold text-neon-blue">{totalEarned}</div>
            ...
            {topUsers.map((user) => (
              <div key={user.rank} className="flex items-center justify-between p-3 bg-cosmic-gray rounded-lg">
                ...
                <div className="text-right">
                  <div className="text-neon-green font-semibold text-sm">{user.earned}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {myTeam.map((team, index) => (
            <div key={index} className="cosmic-card rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-neon-green font-bold">{team.earned}</span>
              </div>
              ...
              <div className="mt-2 bg-cosmic-gray rounded-full h-2">
                <div 
            ...
            {earningsData.map((item, index) => (
              <div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-neon-green font-semibold">{item.amount}</span>
                </div>
                ...
                <div className="text-right text-xs text-gray-400">{item.percentage}%</div>
              </div>
            ))}
          </div>
        </div>
